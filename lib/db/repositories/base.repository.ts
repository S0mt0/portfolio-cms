...
export function createRepository<T extends WithTimestamps>(collectionName: string) {
  ...
  return {
    ...
    async create(data: Omit<T, "createdAt" | "updatedAt">) {
      const now = new Date();
      const payload = { ...data, createdAt: now, updatedAt: now } as OptionalUnlessRequiredId<T>;
      const result = await collection().insertOne(payload);
      return { ...payload, _id: result.insertedId } as WithId<T>;
    },
    async updateOne(filter: Filter<T>, data: Partial<Omit<T, "createdAt" | "updatedAt">>) {
